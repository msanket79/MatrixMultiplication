...
cache:
  ...
  bp_l1_tlb_miss_l2_tlb_miss.if1g
       [The number of instruction fetches that miss in both the L1 and L2 TLBs. Instruction fetches to a 1GB page]
  bp_l1_tlb_miss_l2_tlb_miss.if2m
       [The number of instruction fetches that miss in both the L1 and L2 TLBs. Instruction fetches to a 2MB page]
  bp_l1_tlb_miss_l2_tlb_miss.if4k
       [The number of instruction fetches that miss in both the L1 and L2 TLBs. Instruction fetches to a 4KB page]
  bp_snp_re_sync
       [The number of pipeline restarts caused by invalidating probes that hit on the instruction stream currently being executed. This would happen if the active instruction stream was being modified by
  ...
  ic_cache_inval.fill_invalidated
       [IC line invalidated due to overwriting fill response. The number of instruction cache lines invalidated. A non-SMC event is CMC (cross modifying code), either from the other thread of the core or
        another core]
  ic_cache_inval.l2_invalidating_probe
       [IC line invalidated due to L2 invalidating probe (external or LS). The number of instruction cache lines invalidated. A non-SMC event is CMC (cross modifying code), either from the other thread of the
  ...
  l2_cache_req_stat.ic_dc_hit_in_l2
       [Core to L2 cacheable request access status (not including L2 Prefetch). Instruction cache request hit in L2 and Data cache request hit in L2 (all types)]
  l2_cache_req_stat.ic_dc_miss_in_l2
       [Core to L2 cacheable request access status (not including L2 Prefetch). Instruction cache request miss in L2 and Data cache request miss in L2 (all types)]
  ...
  l2_latency.l2_cycles_waiting_on_fills
       [Total cycles spent waiting for L2 fills to complete from L3 or memory, divided by four. Event counts are for both threads. To calculate average latency, the number of fills from both threads must be
        used]
  l2_pf_hit_l2
  ...
  l2_request_g1.l2_hw_pf
       [All L2 Cache Requests (Breakdown 1 - Common). L2 Prefetcher. All prefetches accepted by L2 pipeline, hit or miss. Types of PF and L2 hit/miss broken out in a separate perfmon event]
...
core:
  ...
  ex_ret_fus_brnch_inst
       [Retired Fused Instructions. The number of fuse-branch instructions retired per cycle. The number of events logged per cycle can vary from 0-8]
  ex_ret_instr
...
floating point:
  ...
  fp_num_mov_elim_scal_op.opt_potential
       [Number of Ops that are candidates for optimization (have Z-bit either set or pass). This is a dispatch based speculative event, and is useful for measuring the effectiveness of the Move elimination and
        Scalar code optimization schemes]
  fp_num_mov_elim_scal_op.optimized
       [Number of Scalar Ops optimized. This is a dispatch based speculative event, and is useful for measuring the effectiveness of the Move elimination and Scalar code optimization schemes]
  fp_num_mov_elim_scal_op.sse_mov_ops
       [Number of SSE Move Ops. This is a dispatch based speculative event, and is useful for measuring the effectiveness of the Move elimination and Scalar code optimization schemes]
  fp_num_mov_elim_scal_op.sse_mov_ops_elim
       [Number of SSE Move Ops eliminated. This is a dispatch based speculative event, and is useful for measuring the effectiveness of the Move elimination and Scalar code optimization schemes]
  fp_ret_sse_avx_ops.add_sub_flops
       [Add/subtract FLOPS. This is a retire-based event. The number of retired SSE/AVX FLOPS. The number of events logged per cycle can vary from 0 to 64. This event can count above 15]
  fp_ret_sse_avx_ops.all
       [All FLOPS. This is a retire-based event. The number of retired SSE/AVX FLOPS. The number of events logged per cycle can vary from 0 to 64. This event can count above 15]
  fp_ret_sse_avx_ops.div_flops
       [Divide/square root FLOPS. This is a retire-based event. The number of retired SSE/AVX FLOPS. The number of events logged per cycle can vary from 0 to 64. This event can count above 15]
  fp_ret_sse_avx_ops.mac_flops
       [Multiply-add FLOPS. Multiply-add counts as 2 FLOPS. This is a retire-based event. The number of retired SSE/AVX FLOPS. The number of events logged per cycle can vary from 0 to 64. This event can count
        above 15]
  fp_ret_sse_avx_ops.mult_flops
       [Multiply FLOPS. This is a retire-based event. The number of retired SSE/AVX FLOPS. The number of events logged per cycle can vary from 0 to 64. This event can count above 15]
  ...
  fp_retired_ser_ops.sse_ctrl_ret
       [The number of serializing Ops retired. SSE control word mispredict traps due to mispredictions in RC, FTZ or DAZ, or changes in mask bits]
...
memory:
  ls_bad_status2.stli_other
       [Non-forwardable conflict; used to reduce STLI's via software. All reasons. Store To Load Interlock (STLI) are loads that were unable to complete because of a possible match with an older store, and the
  ...
  ls_inef_sw_pref.mab_mch_cnt
       [The number of software prefetches that did not fetch data outside of the processor core. Software PREFETCH instruction saw a match on an already-allocated miss request buffer]
  ls_int_taken
  ...
  ls_locks.bus_lock
       [Retired lock instructions. Bus lock when a locked operations crosses a cache boundary or is done on an uncacheable memory type. Comparable to legacy bus lock]
  ...
  ls_pref_instr_disp.prefetch
       [Software Prefetch Instructions Dispatched (Speculative). Prefetch_T0_T1_T2. PrefetchT0, T1 and T2 instructions. See docAPM3 PREFETCHlevel]
  ...
  ls_refills_from_sys.ls_mabresp_lcl_cache
       [Demand Data Cache Fills by Data Source. Hit in cache; local CCX (not Local L2), or Remote CCX and the address's Home Node is on this thread's die]
...
other:
  ...
  de_dis_dispatch_token_stalls1.fp_misc_rsrc_stall
       [Cycles where a dispatch group is valid but does not get dispatched due to a token stall. FP Miscellaneous resource unavailable. Applies to the recovery of mispredicts with FP ops]
  de_dis_dispatch_token_stalls1.fp_reg_file_rsrc_stall
       [Cycles where a dispatch group is valid but does not get dispatched due to a token stall. Floating point register file resource stall. Applies to all FP ops that have a destination register]
  de_dis_dispatch_token_stalls1.fp_sch_rsrc_stall
       [Cycles where a dispatch group is valid but does not get dispatched due to a token stall. FP scheduler resource stall. Applies to ops that use the FP scheduler]
  de_dis_dispatch_token_stalls1.int_phy_reg_file_token_stall
       [Cycles where a dispatch group is valid but does not get dispatched due to a token stall. Integer Physical Register File resource stall. Applies to all ops that have an integer destination register]
  de_dis_dispatch_token_stalls1.int_sched_misc_token_stall
       [Cycles where a dispatch group is valid but does not get dispatched due to a token stall. Integer Scheduler miscellaneous resource stall]
  de_dis_dispatch_token_stalls1.load_queue_token_stall
       [Cycles where a dispatch group is valid but does not get dispatched due to a token stall. Load queue resource stall. Applies to all ops with load semantics]
  de_dis_dispatch_token_stalls1.store_queue_token_stall
       [Cycles where a dispatch group is valid but does not get dispatched due to a token stall. Store queue resource stall. Applies to all ops with store semantics]
